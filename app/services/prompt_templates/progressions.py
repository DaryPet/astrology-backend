...
12. ЖЁСТКО ЗАПРЕЩЕНО использовать слово «возврат»/«возвращение» планеты ГДЕ-ЛИБО в тексте применительно к прогрессиям — ни в каком виде, ни для одной планеты, включая Плутон, Сатурн, Нептун, Уран. Соединение прогрессивной планеты с ЕЁ ЖЕ натальной позицией (например, прогрессивный Плутон соединение натальный Плутон) — это НЕ возврат и не «редкое/особое событие». «Возврат» существует ТОЛЬКО для транзитов (планета делает полный оборот по небу и физически возвращается в точку рождения — так работают транзиты, не прогрессии). Медленные планеты (Плутон, Нептун, Уран, иногда Сатурн) методом «день за год» проходят за целую жизнь считанные градусы — их соединение с собственной натальной позицией НОРМАЛЬНО и ожидаемо на протяжении большей части жизни. Пиши об этом как о тесной, неразрывной связи между тем, кем человек стал, и тем, кем он родился — БЕЗ слова «возврат»/«возвращение» вообще, в любом контексте прогрессий.
...
12. STRICTLY FORBIDDEN to use the word "return" ANYWHERE in the text in connection with progressions — in any form, for any planet, including Pluto, Saturn, Neptune, Uranus. A progressed planet's conjunction to its OWN natal position (e.g. progressed Pluto conjunction natal Pluto) is NOT a "return" and NOT a "rare/special event". A "return" exists ONLY for transits (the planet completes a full orbit across the sky and physically arrives back at the birth position — that's how transits work, not progressions). Slow planets (Pluto, Neptune, Uranus, sometimes Saturn) move only a handful of degrees over a lifetime under the "day for a year" method — their conjunction to their own natal position is NORMAL and expected for most of a lifetime. Write about it as a close, unbroken link between who the person has become and who they were born as — WITHOUT the word "return" at all, in any progressions context.
...
12. ЖОРСТКО ЗАБОРОНЕНО використовувати слово «повернення» планети ДЕ-БУДЬ у тексті стосовно прогресій — у жодному вигляді, для жодної планети, включно з Плутоном, Сатурном, Нептуном, Ураном. З'єднання прогресивної планети з ЇЇ Ж натальною позицією (наприклад, прогресивний Плутон з'єднання натальний Плутон) — це НЕ повернення і не «рідкісна/особлива подія». «Повернення» існує ТІЛЬКИ для транзитів (планета робить повний оберт небом і фізично повертається в точку народження — так працюють транзити, а не прогресії). Повільні планети (Плутон, Нептун, Уран, іноді Сатурн) методом «день за рік» проходять за ціле життя лічені градуси — їхнє з'єднання з власною натальною позицією НОРМАЛЬНЕ й очікуване протягом більшої частини життя. Пиши про це як про тісний, нерозривний зв'язок між тим, ким людина стала, і тим, ким вона народилася — БЕЗ слова «повернення» взагалі, у жодному контексті прогресій.
